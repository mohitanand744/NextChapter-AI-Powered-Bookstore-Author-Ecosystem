    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="relative text-center">
        <div className="relative flex flex-col items-center justify-center">
          ...
          >
            {!badge && <BiCodeAlt className="text-base" />}
            {badge || "Backend APIs in Progress"}
          </motion.div>
          ...
          <motion.p
            ...
            className="mb-6 max-w-xl text-sm font-medium leading-7 text-tan/80 sm:text-base text-center"
          >
          ...
          >
            {features ? (
              features.map((feature, idx) => (
                <div key={idx} className="rounded-2xl border border-tan/15 bg-white/5 px-4 py-3 flex items-center gap-3">
                  <HiSparkles className="text-yellow-600/70 text-md flex-shrink-0" />
                  <p className="text-[14px] leading-5 text-tan/80 font-semibold">{feature}</p>
                </div>
              ))
            ) : (
              <>
          ...
          {(exploreLink || onExplore) ? (
            ...
            <div className="flex w-full flex-col items-center justify-center">
              {footer && (
                <p className="mb-4 text-xs tracking-wider uppercase text-tan/50 font-bold">
                  {footer}
                </p>
              )}
              <Button
